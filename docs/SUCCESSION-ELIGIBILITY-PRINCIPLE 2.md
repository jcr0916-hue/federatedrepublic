...
## ABSOLUTE-BAR vs. ACTING-ALLOWED DECISION (260713) — absolute bar, both triggers

John's question: should an LC-ineligible Speaker be able to ACT as LC during a temporary
incapacity (§2.9(6)) but not INHERIT full authority through permanent succession (§2.9(1))? He
likes the absolute bar but is a pragmatist about continuity.

**Decision: ABSOLUTE BAR carries the day — the ineligible person is skipped for BOTH full
succession AND temporary acting authority.** Reasoning:

- **The "temporary" path is the MORE dangerous one, not the safer one.** Full succession happens
  once (LC dies/resigns). Temporary acting can RECUR — a loyal same-faction sitting LC can be
  "temporarily incapacitated" repeatedly, handing the former LC real executive authority in rolling
  increments. Allowing acting-but-not-full reopens a RENEWABLE version of the Putin exploit. The
  temporary nature is not a safeguard here; it's an attack surface.
- **The absolute bar's virtue is that it's absolute.** "Ineligible for full but eligible for acting"
  creates a seam (is this acting or full? can acting be extended? what if incapacity is never
  resolved?) — reintroducing the interpretive surface a clean bar eliminates. §2.1 "single,
  non-renewable" reads cleanest as: does not exercise LC authority again, by any path, in any amount.
- **Continuity is already solved without routing power through the barred person.** Skip -> the role
  passes to the next eligible person. The pragmatic need (someone competent governs during an
  incapacity) is met by the senior Senator, NOT by the former LC. John's pragmatism is honored, just
  relocated: the fix is "give the acting role a proper fallback," not "let the ineligible person act."

**CRITICAL DRAFTING REQUIREMENT surfaced by checking the text:**
- §2.9(2) provides a senior-Senator floor for FULL SUCCESSION. Good — skipping an ineligible Speaker
  routes cleanly there.
- BUT §2.9(6) (temporary acting) has its OWN fallback and it is SPEAKER-ONLY: "where the Speaker
  ceases to be a Senator, acting authority passes to the Senate Speaker then in office." There is NO
  senior-Senator floor for the ACTING case. So applying the absolute bar to §2.9(6) as-written would
  leave no defined next-in-line for the acting role = a temporary vacuum.
- **THE FIX:** the amendment must extend §2.9(6)'s fallback to match §2.9(2) — e.g. "where the Senate
  Speaker is ineligible to hold the office of Legat Consul, or is otherwise unable to serve, Acting
  Legat Consul authority transfers to the most senior Senator by continuous service." This closes the
  vacuum AND fixes a latent asymmetry (the acting-fallback is currently thinner than the
  succession-fallback regardless of the eligibility question — arguably worth fixing on its own).

**Net:** absolute bar for both triggers + extend §2.9(6) fallback to the senior-Senator floor.
Satisfies absolutist (bright line, no seam, no renewable exploit) AND pragmatist (continuity
guaranteed by explicit fallback). The split was rejected: it trades the bright line for a continuity
problem that is better solved directly by giving the acting role a proper floor.
