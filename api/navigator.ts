// Vercel Edge Function: Constitution Navigator
// Adapted from netlify/edge-functions/navigator.ts
// Changes: added export const runtime = 'edge'; changed Deno.env.get → process.env
...
let cachedProvisions: any[] | null = null;
...
async function getProvisions() {
  ...
  cachedProvisions = data.flatMap((a: any) => a.provisions);
  return cachedProvisions;
}

function expandQuery(raw: string) {
  const words = raw.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter((w: string) => w && !STOP.has(w));
  const expanded = new Set(words);
  const lc = raw.toLowerCase();
  for (const [phrase, syns] of Object.entries(SYNONYMS)) {
    if (lc.includes(phrase)) (syns as string[]).forEach((s: string) => s.split(' ').forEach((w: string) => expanded.add(w)));
  }
  for (const word of words) {
    if ((SYNONYMS as any)[word]) ((SYNONYMS as any)[word] as string[]).forEach((s: string) => s.split(' ').forEach((w: string) => expanded.add(w)));
  }
  return [...expanded].filter((w: string) => w.length > 1);
}

function scoreProvision(prov: any, terms: string[]) {
...
function topProvisions(provisions: any[], terms: string[], n = 3) {
  return provisions
    .map((p: any) => ({ p, s: scoreProvision(p, terms) }))
    .filter((x: any) => x.s > 0)
...
function topScenarios(terms: string[], n = 2) {
  return SCENARIOS
...
export default async (request: Request) => {
  ...
  try {
    ...
    const matched = topProvisions(provisions as any[], terms, 3);
    ...
    const provisionContext = matched.map((p: any) => `[${p.num}] ${p.name}\n${p.text}`).join('\n\n');
    ...
    return jsonResp({
      summary,
      provisions: matched.map((p: any) => ({ num: p.num, name: p.name })),
      scenarios
    });
