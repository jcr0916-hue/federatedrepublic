...
export function planIngest({root,inbox}) {
  ...
  for(const item of worlds) {
    if(item.metadata.worldSeq===undefined || item.metadata.worldSeq===null || item.metadata.worldSeq==='')
  ...
  for(const item of plan.items) {
    ...
    if(item.record && [...published,...worlds.filter(i=>i!==item).map(i=>i.record)].some(r=>r.data.worldSeq===item.metadata.worldSeq)) item.errors.push('Duplicate worldSeq in repository or inbox.');
  }
  const options={
