import {registryWarnings} from './world-chronology.mjs';
...
export function checkWorldPublishing(records,{provisions,dossiers,status,registry}) {
  ...
  for(const {inputPath,data:d,content} of records){
    ...
    if((d.worldKind!=='nrs'||d.worldSeq!==undefined)&&(!Number.isSafeInteger(d.worldSeq)||d.worldSeq<1))fail('worldSeq must be a positive integer');
  ...
  const maxSeq=Math.max(0,...records.map(p=>p.data.worldSeq).filter(Number.isSafeInteger));
  ...
  const nrsFrontier=status?.match(/nrsSeq through \*\*(\d+)\*\*/i);
  const maxNrs=Math.max(0,...records.filter(p=>p.data.worldKind==='nrs').map(p=>p.data.nrsSeq));
  if(maxNrs && (!nrsFrontier||Number(nrsFrontier[1])!==maxNrs))warnings.push(`Review WORLD-STORY-STATUS.md: NRS sequence frontier is ${maxNrs}.`);
  const unreviewed=records.filter(p=>p.data.worldKind==='nrs'?(!nrsFrontier||p.data.nrsSeq>Number(nrsFrontier[1])):(!frontier||p.data.worldSeq>Number(frontier[1])));
  ...
  if(registry){
    try {warnings.push(...registryWarnings(records,registry,status));}
