...
const WORLD_FILE_RE=/^torenthia-(?:(?:news|nrs|sc)-\d+|dispatch[^.]*)\.html$/i;
...
export function nextNrsSeq(items){
  return Math.max(0,...items.map(x=>x.data.nrsSeq).filter(Number.isSafeInteger))+1;
}
...
export function buildWorldDraft(opts,items){
  ...
  const seq=opts.kind==='nrs'?nextNrsSeq(items):nextWorldSeq(items);
  if(opts.kind==='nrs' && !/^NRS-Y\d+-\d{4,}$/.test(opts.nrsId||''))throw Error('--nrs-id must be an explicitly reviewed NRS-Y13-0706 style reference');
  if(opts.kind==='nrs' && items.some(x=>x.data.nrsId===opts.nrsId))throw Error('Duplicate nrsId');
  ...
  const lines=[
    "---",
    `worldKind: ${opts.kind}`,
    ...(opts.kind==='nrs'?[`nrsSeq: ${seq}`,`nrsId: ${JSON.stringify(opts.nrsId)}`]:[`worldSeq: ${seq}`]),
