...
for article in data:
    ...
    raise RuntimeError('§2.16 not found')
data_path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

p = root / 'scenario-the-twenty-four-hours.html'
s = p.read_text(encoding='utf-8')
old_desc = ('The Council of Ministers determines that the Civic Consul is temporarily unable to exercise authority. He disagrees. Under §2.16 he does not need to persuade a court, a chamber, or a doctor — he publishes a declaration and, twenty-four hours later, he is Consul again. The provision hands him that door on purpose. A repeat determination on the same or substantially similar grounds within 90 days changes that procedure.')
new_desc = ('The Council of Ministers determines that the Civic Consul is temporarily unable to exercise authority. He restores himself once after twenty-four hours. When the same incapacity recurs inside ninety days, the Council must reach two-thirds, restoration requires broader agreement, and the SC becomes available to police misuse.')
s = s.replace(old_desc, new_desc)
s = s.replace('<span class="prov-tag">§2.16(3) Restoration &amp; the 90-Day Brake</span>', '<span class="prov-tag">§2.16(2)–(5) Restoration &amp; Recurring Incapacity</span>')
s = s.replace('<span class="prov-tag">§2.16(4) Declination</span>', '<span class="prov-tag">§2.16(7) Declination</span>')
s = s.replace('If fewer than five Ministers were in office, the other Consul and Chief Justice could instead jointly determine incapacity and publish it to the NRS. Here the Council of Ministers has eleven members in office.', 'If fewer than five Council members were available, legislators selected by lot would fill the deciding body to five — Senators for a Civic Consul proceeding, Assembly members for a Legat Consul proceeding. Here the Council of Ministers has eleven members in office.')
s = s.replace('§2.16(2) — The Determination', '§2.16(1) — The Determination')
s = s.replace('The Council may determine incapacity by majority of members then in office, provided no fewer than five are in office.', 'The Council may determine incapacity by majority, with at least five participating.')
s = s.replace('Read the clause carefully, because it is doing something precise: <em>this function is a constitutional function of the Council and is not an exercise of executive authority.</em> That sentence is the whole reason the mechanism is safe. The Council of Ministers is not a cabinet in the ordinary sense — it does not hold executive power and cannot take any by finding a Consul unable to use his. The Ministers are not seizing the office. They are making a finding of fact about a person, in the same way a clerk certifies a signature, and the finding does not put a single unit of power into their hands.', 'Read the mechanics carefully. The Council makes the incapacity determination, but the determination gives the Council no executive authority. Upon publication, authority goes immediately to the successor already fixed by the Constitution. The Ministers are not seizing the office; they are triggering a succession rule written before any of them formed a view about Aurel Vasko.')
s = s.replace('BY: Council of Ministers, sitting as a constitutional function under §2.16(2)(a)', 'BY: Council of Ministers under §2.16(1)')
s = s.replace('MEMBERS IN OFFICE: 11 &nbsp;&nbsp; PRESENT: 11 &nbsp;&nbsp; THRESHOLD: majority of members in office (6), minimum 5 in office — MET', 'MEMBERS AVAILABLE: 11 &nbsp;&nbsp; PARTICIPATING: 11 &nbsp;&nbsp; THRESHOLD: majority (6), minimum 5 participating — MET')
s = s.replace('This determination is not an exercise of executive authority (§2.16(2)(a)).', 'The Council receives no executive authority from the determination.')
...
s = s.replace('I declare that the incapacity determined under §2.16(2) has ended.', 'I declare that the incapacity determined under §2.16(1) has ended.')
s = s.replace('And the twenty-four hours are not decoration. They provide an ordinary restoration interval so that the Speakers, the other Consul, and the acting officer — all of whom must be transmitted the declaration — have one full day to look at the man and decide what to do about it. The clause does not tell them what to do. It gives them a day and their own judgement, which is all the Constitution ever gives anybody.', 'And the twenty-four hours are not decoration. They are the transition interval between the restoration declaration and the return of authority. The acting government has a known endpoint, the public record has a timestamp, and the Republic does not switch executive authority in the same instant that one person files a sentence.')
replacement = '''      <p class="panel-num">Panel Four</p>
      ...
      <div class="beat">
        ...
        <p>The Council expects recovery to take longer than a month. The Assembly and Senate may each, by simple majority, authorize a specified longer period, never beyond six months from the heightened determination.</p>
...
s, n = re.subn(r'      <p class="panel-num">Panel Four</p>.*?      <div class="scenario-nav">', replacement, s, flags=re.S)
if n != 1:
...
s = p.read_text(encoding='utf-8')
s = s.replace('how an incapacity is determined, how acting authority vests, how a Consul ends it, and what changes if the same grounds recur inside ninety days.', 'how incapacity is determined, how acting authority vests, how restoration works, and how repeated incapacity is handled inside ninety days.')
...
if n != 1:
    raise RuntimeError(f'Expected one NRS replacement, got {n}')
p.write_text(s, encoding='utf-8')
...
if entry not in s:
    if marker not in s:
        raise RuntimeError('updates insertion point not found')
